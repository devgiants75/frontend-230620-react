...
function App() {
  ...
  const connectionTest = () => {
    mockAxios // mockAxios 사용
    .get('http://localhost:4040/') // 해당 주소로 GET 요청을 보냄.
    .then((response) => {
  ...
  useEffect(() => {
    if (token && !user) { // 토큰이 있고 사용자 정보가 없을 때
      // 
      mockAxios.get('http://localhost:4040/api/user/', { headers: {
    ...
    if (!token && user) { // 토큰이 없고 사용자 정보만 있을 때
      // 사용자 정보를 null로 지정
      setUser(null);
    }
